...
// commonZones is a curated list of widely-used GCP zones, bundled so that
// shell completion and error hints work without a network call.
...
func AddZoneFlag(cmd *cobra.Command) {
	...
	_ = cmd.RegisterFlagCompletionFunc("zone", func(_ *cobra.Command, _ []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		...
		for _, z := range commonZones {
			if strings.HasPrefix(z, toComplete) {
